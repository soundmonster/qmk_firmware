...
//Tap dance enums
enum {
  RACL = 0, // right alt/colon
  LSCS = 1, // space cadet shift on steroids: hold for shift, tap for (, double tap for [, triple tap for {
  RSCS = 2, // space cadet shift on steroids: hold for shift, tap for ), double tap for ], triple tap for }
  LWR  = 3, // hold for lower, tap and hold for raise
  RSE  = 4, // hold for raise, tap and hold for lower
};
...
/* void rse_reset(qk_tap_dance_state_t *state, void *user_data); */
...
void racl_reset(qk_tap_dance_state_t *state, void *user_data) {
  switch(racl_tap_state.state) {
    case SINGLE_TAP: /* we've sent the string already */ break;
